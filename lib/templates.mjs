 ...
 *   - CodeKit-friendly .kit partials in src/html/ (compiled locally to public/)
 ...
 * since CodeKit-compiled HTML is checked in (Firebase Hosting deploys from it).
...
npm run build           # less + js → public/
\`\`\`

CodeKit handles \`.kit\` → \`.html\` compilation locally. Compiled HTML is
committed to \`public/\` so CI / Firebase deploy doesn't need CodeKit.
